...
class GobbletGUI:
    ...
    def create_widgets(self):
        ...
        self.lbl_status = tk.Label(main_frame, text="Tocca a te (Blu)", font=("Arial", 16), fg="blue", bg="#f0f0f0")
        ...
        tk.Label(left_col, text="Tuoi Pezzi", font=("Arial", 12, "bold"), bg="#e0e0e0").pack(pady=5)
        ...
        tk.Label(right_col, text="Pezzi AI", font=("Arial", 12, "bold"), bg="#e0e0e0").pack(pady=5)
    ...
    def ask_game_settings(self):
        ...
        setup_win.title("Impostazioni Partita")
        setup_win.geometry("350x350")
        setup_win.resizable(False, False)
        
        setup_win.transient(self.root) 
        setup_win.grab_set() 

        tk.Label(setup_win, text="CONFIGURAZIONE GOBBLET", font=("Helvetica", 16, "bold"), fg="#333").pack(pady=15)
      
        tk.Label(setup_win, text="Chi fa la prima mossa?", font=("Arial", 11, "bold")).pack(pady=5)
        
        var_starter = tk.StringVar(value="user") 
        
        frame_starter = tk.Frame(setup_win)
        frame_starter.pack()
        tk.Radiobutton(frame_starter, text="Tu (Blu)", variable=var_starter, value="user", font=("Arial", 10)).pack(anchor="w")
        tk.Radiobutton(frame_starter, text="AI (Rosso)", variable=var_starter, value="ai", font=("Arial", 10)).pack(anchor="w")
        

        tk.Label(setup_win, text="Livello Intelligenza AI", font=("Arial", 11, "bold")).pack(pady=(15, 5))
        
        var_diff = tk.IntVar(value=2) 
        
        frame_diff = tk.Frame(setup_win)
        frame_diff.pack()
        tk.Radiobutton(frame_diff, text="Facile (Veloce)", variable=var_diff, value=1, font=("Arial", 10)).pack(anchor="w")
        tk.Radiobutton(frame_diff, text="Medio (Bilanciato)", variable=var_diff, value=2, font=("Arial", 10)).pack(anchor="w")
        tk.Radiobutton(frame_diff, text="Difficile (Lento - Depth 3)", variable=var_diff, value=3, font=("Arial", 10)).pack(anchor="w")
        
  
        def on_confirm():
            self.ai_depth = var_diff.get()
            starter = var_starter.get()
            
            print(f"Impostazioni salvate: Starter={starter}, Depth={self.ai_depth}")

            setup_win.destroy()
       
            if starter == "user":
                self.turn = 'w'
                self.lbl_status.config(text=f"Tocca a te (Difficoltà: {self.ai_depth})", fg="blue")
            else:
                self.turn = 'b'
                self.lbl_status.config(text=f"L'AI (Difficoltà: {self.ai_depth}) sta pensando...", fg="red")
        ...
        tk.Button(setup_win, 
                  ...
                  width=20, 
                  height=2,
                  command=on_confirm).pack(pady=30)
    ...
    def on_hand_click(self, index):
        ...
        piece_str = self.hand_w[index] 
        size = piece_str.split(',')[1][0]
        
        self.selected_source = ('hand', index, piece_str)
        self.hand_buttons_w[index].config(bg="#ffff99") 
    ...
    def reset_selection(self):
        ...
        self.selected_source = None
        self.selected_btn = None
        self.lbl_status.config(text="Tocca a te (Blu)")
    ...
    def ai_turn(self):
        ...
        print(f"AI Thinking at Depth {self.ai_depth}...")
        query = f"cerca_mossa_migliore(stato({board_s}, {hand_w_s}, {hand_b_s}, b), {self.ai_depth}, Mossa, _)"
        try:
            res = list(self.prolog.query(query))
            if res:
                best_move = str(res[0]['Mossa'])
                print(f"AI Move Raw: {best_move}") 
                if "gioca_da_mano" in best_move:
                    ...
                    if match:
                        ...
                        print(f"AI mette {piece} in cella {idx_board+1}") 
                ...
                if self.check_winner(): return
                self.turn = 'w'
                self.lbl_status.config(text="Tocca a te (Blu)", fg="blue")
            else:
